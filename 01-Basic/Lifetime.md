# 图解 Rust 生命周期：引用到底能活多久？

> 为什么编译器总是抱怨“引用存活时间不够长”？  
> 生命周期标注是在“延长”变量的寿命吗？还是在向编译器“证明”某种安全事实？

在 C/C++ 中，悬垂指针（Dangling Pointer）是无数调试噩梦的源头。Rust 的生命周期（Lifetime）机制并非为了增加复杂度，而是为了在**编译阶段**彻底消灭这类内存灾难。
