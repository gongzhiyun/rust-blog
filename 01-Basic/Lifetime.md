...
即便我们显式标注了生命周期，也不能延长数据的存活时间。编译器始终基于**真实的物理作用域**进行检查。

**子集约束（Subset Constraint）**依然是铁律：
- **'b (Owner)**：数据 `x` 的实际生命周期（较短）。
- **'a (Reference)**：引用 `r` 的实际生命周期（较长）。
- **检查逻辑**：编译器要求 **'a ⊆ 'b**（引用必须活在数据腹中）。但在上述代码中，显然 `'a` 超出了 `'b` 的范围，因此被无情拒绝。

> **Deep Insight**: **生命周期参数不是在改变数据的寿命，而是在签署“生死状”。**
> 当你标注 `&'a T` 时，你并不是在告诉编译器“请把 T 活得久一点”，而是在向编译器保证：“我手中的这个引用，绝不会比 T 活得更久。”这是一种**只做减法、不做加法**的契约。
