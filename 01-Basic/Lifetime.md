...
> 生命周期（Lifetime）是 Rust 借用检查器的核心，用于在编译期验证引用的合法性。它描述的是引用与其背后的数据之间客观存在的作用域包含关系。在 C/C++ 中，悬垂指针（Dangling Pointer）是无数调试噩梦的源头，而 Rust 的生命周期机制并非为了增加复杂度，而是为了在**编译阶段**彻底消灭这类内存灾难。
