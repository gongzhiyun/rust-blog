...
### 图解分析
- **父查子**：对于索引为 `i` 的节点，其左子节点在 `2i + 1`，右子节点在 `2i + 2`。
- **子查父**：对于索引为 `i` 的节点，其父节点在 `(i - 1) / 2`（整数除法自动取整）。
- **映射关系**：这种映射确保了数组中没有空洞，内存利用率达到 100%。
...
### 图解分析
- **末位追加**：新元素 30 插入到索引 2（数组末尾），保持了“完全二叉树”的物理结构。
- **气泡上浮**：30 与父节点 20 比较，发现 30 更大，于是交换位置。
- **时间复杂度**：$O(\log N)$，因为最大交换次数等于树的高度。

---

## 4. 出堆行为：下沉 (Sift Down)

弹出堆顶（最大值）后，为了填补空缺且不破坏数组的连续性，我们将末尾元素移到堆顶，然后让它向下“沉降”。

![BinaryHeap Sift Down](./imgs/binary_heap_sift_down.svg)

### 图解分析
- **末位替补**：将数组最后一个元素 10 移动到堆顶，移除原最大值。
- **强者生存**：10 与两个子节点（20, 15）比较，与其中较大的子节点（20）交换。
- **递归下沉**：重复此过程，直到节点不再小于子节点或成为叶子节点。
