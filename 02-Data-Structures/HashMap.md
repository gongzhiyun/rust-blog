...
*   **Stack (栈)**：
    *   `map` 结构体非常轻量，核心是一个指向 Heap 的 `ctrl` 指针。
    *   `bucket_mask` 用于快速计算索引（`hash & mask`）。
    *   `items` 记录元素数量，`growth_left` 记录剩余容量（考虑负载因子）。
*   **Heap (堆)**：
    *   **Control Bytes**：每个 bucket 对应 1 个字节的元数据。
        *   `EMPTY` (0xFF)：空闲。
        *   `DELETED` (0x80)：已删除（Tombstone）。
        *   `FULL` (0x00..0x7F)：存储 Hash 值的最高 7 位（H2）。
    *   **Slots**：实际存储 `(Key, Value)` 对。
    *   **布局优势**：查找时只需扫描紧凑的 Control Bytes（极高的缓存命中率），只有匹配成功时才去访问较大的 Slots 数据。
