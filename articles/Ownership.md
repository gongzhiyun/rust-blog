# 图解 Rust 所有权： 移动语义、借用检查与内存安全的底层机制

所有权（Ownership）是 Rust 在编译期实现内存安全的核心机制。它通过一套清晰的规则，杜绝了悬垂指针与内存泄漏。

## 内存管理的挑战：安全与性能的权衡

在所有权系统出现之前，开发者往往需要在“手动管理的灵活性”与“自动垃圾回收的安全性”之间做出艰难选择。手动管理（如 C/C++）极易引入难以追踪的内存隐患。
